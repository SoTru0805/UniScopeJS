      ...
      <main className="flex flex-col items-center justify-start p-4 md:p-8 lg:p-12 bg-background">
        <div className="w-full max-w-5xl space-y-8"> {/* Increased max-width */}
          <header className="text-center space-y-2 mb-8">
            ...
            <h1 className="text-3xl md:text-4xl font-bold text-primary">Submit and Discover reviews</h1> {/* Changed from UniReview */}
          ...
          <Card className="shadow-md"> {/* Wrap review list/summary in a Card */}
            ...
            <CardContent className="space-y-4">
              {fetchError ? (<Alert variant="destructive" className="flex items-center gap-3"><AlertTriangle className="h-6 w-6" /><div><AlertTitle>Error Loading Reviews</AlertTitle><AlertDescription>{fetchError}</AlertDescription></div></Alert>) : reviews.length === 0 ? (<Alert className="border-primary/30 bg-accent/50 flex items-center gap-3"><Info className="h-6 w-6 text-primary" /><div><AlertTitle className="text-primary">No Reviews Yet</AlertTitle><AlertDescription>Be the first to submit a review using the form above!</AlertDescription></div></Alert>) : (<ReviewList reviews={reviews} unitCode={defaultUnitCode} />)}
            </CardContent>
          </Card>
        </div>
      </main>
